...
examples.svg = function() {
  ...
  svg = new_svg(xlim=xrange,ylim=yrange) %>%
    svg_xaxis(axis.offset = 10) %>%
    svg_yaxis(axis.offset = 10) %>%
...
new_svg = function(width=500, height=400, xlim=c(0,1),ylim=xlim) {
  restore.point("svg")

  svg = new.env()
  svg$head = paste0("<svg width='",width,"' height='",height,"'>")
  svg$width = 500
  svg$height = height
  svg$dr = make.domain.range(xlim=xlim,ylim=ylim,width=width, height=height)
  svg$el = NULL
...
make.domain.range = function(xlim,ylim,width=500, height=400, margins=c(bottom, left, top, right), bottom=height / 10, top = height/10, left = width/8, right=width/20) {
  list(
    ...
    range = list(
      x=c(margins[2],width-margins[2]-margins[4]),
      y=rev(c(margins[1],height-margins[1]-margins[3]))
    )
  )
}

domain.to.range = function(x=NULL,y=NULL,domain=dr$domain, range=dr$range,dr=svg$dr, svg=NULL) {
...
svg_polyline = function(svg, x,y,id=NULL, class="polyline",style=c(nlist(fill, stroke,stroke_width), extra.style), fill="none", stroke="black",stroke_width=NULL, extra.style=list(), level=0, tooltip=NULL) {
  restore.point("svg_polyline")
  rp = domain.to.range(x=x,y=y,svg=svg)
  points = paste0(rp$x,",",rp$y, collapse=" ")
  el = svg_tag("polyline", nlist(points,id,class,style), tooltip=tooltip)
  svg_add(svg,el,id,level=level)
}

svg_boxed_label = function(svg, x,y, text,id=NULL, class="boxed-label",style=c(nlist("font-size"=font_size), extra.style), font_size=NULL, extra.style=list(), level=1, tooltip=NULL) {
...
svg_xaxis = function(svg, id="xaxis",
  align="default", dr=svg$dr, return.string=FALSE, level=100, num.ticks=5, ticks =pretty(dr$domain$x, n=num.ticks), tick.size = 10, arrow=FALSE,
  class.group= "axis x-axis",  class.line="axis-main", class.tick="axis-tick",class.tick.label="axis-ticklabel", class.label="axis-label",
  style.line=NULL, style.tick=NULL,style.tick.label=NULL, style.label=NULL, axis.offset=10  ) {
  ...
  ti.lab = paste0('<text x="',x.ticks,'" y="',y2.tick+3,'" ', html_arg_str(style=style.tick.label, class=class.tick.label),' text-anchor="middle" alignment-baseline="hanging">',ticks,"</text>")

  inner = c(line, ti.str, ti.lab)
...
svg_yaxis = function(svg, id="yaxis",
  align="default", dr=svg$dr, return.string=FALSE, level=100, num.ticks=5, ticks =pretty(dr$domain$y, n=num.ticks), tick.size = 10, arrow=FALSE, axis.offset=10,
